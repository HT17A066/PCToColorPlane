...
void revisePlane(pcl::PointCloud<PointType>::Ptr cloud, pcl::ModelCoefficients::Ptr coefficients,
...
static const char* pctocolorplane_spec[] =
{
  "implementation_id", "PCToColorPlane",
  "type_name",         "PCToColorPlane",
  "description",       "Extract planes from point cloud with color",
  "version",           "1.0.0",
  "vendor",            "MasutaniLab",
  "category",          "PointCloud",
  ...
  "conf.default.planePointSizeMin", "100",
  ...
  "conf.default.samePlaneCenterDistanceLimit", "0.02",
  "conf.default.samePlaneRadiusDistanceLimit", "0.01",
  "conf.default.samePlaneHueDistanceLimit", "23",
  "conf.default.planeAccumulationMin", "10",
  ...
  "conf.__widget__.planePointSizeMin", "text",
  ...
  "conf.__widget__.samePlaneCenterDistanceLimit", "text",
  "conf.__widget__.samePlaneRadiusDistanceLimit", "text",
  "conf.__widget__.samePlaneHueDistanceLimit", "text",
  "conf.__widget__.planeAccumulationMin", "text",
  ...
  "conf.__type__.planePointSizeMin", "int",
  ...
  "conf.__type__.samePlaneCenterDistanceLimit", "float",
  "conf.__type__.samePlaneRadiusDistanceLimit", "float",
  "conf.__type__.samePlaneHueDistanceLimit", "float",
  "conf.__type__.planeAccumulationMin", "int",
 ...
 */
PCToColorPlane::PCToColorPlane(RTC::Manager* manager)
// <rtc-template block="initializer">
    : RTC::DataFlowComponentBase(manager),
    m_pcIn("pc", m_pc),
    m_planeOut("plane", m_plane)
 ...
 * @brief destructor
 */
PCToColorPlane::~PCToColorPlane()
{
}



RTC::ReturnCode_t PCToColorPlane::onInitialize()
{
    ...
    bindParameter("planePointSizeMin", m_planePointSizeMin, "100");
    ...
    bindParameter("samePlaneCenterDistanceLimit", m_samePlaneCenterDistanceLimit, "0.02");
    bindParameter("samePlaneRadiusDistanceLimit", m_samePlaneRadiusDistanceLimit, "0.01");
    bindParameter("samePlaneHueDistanceLimit", m_samePlaneHueDistanceLimit, "23");
    bindParameter("planeAccumulationMin", m_planeAccumulationMin, "10");
...
/*
RTC::ReturnCode_t PCToColorPlane::onFinalize()
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onStartup(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onShutdown(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
RTC::ReturnCode_t PCToColorPlane::onActivated(RTC::UniqueId ec_id)
{
...
RTC::ReturnCode_t PCToColorPlane::onDeactivated(RTC::UniqueId ec_id)
{
...
RTC::ReturnCode_t PCToColorPlane::onExecute(RTC::UniqueId ec_id)
{
    ...
    for (int i = 0; i < m_findingIterationLimit; i++) {
            ...
            seg.setOptimizeCoefficients(true);
            //seg.setAxis(axis);
            seg.setAxis(Eigen::Vector3f(0,0.8910065242,0.4539904997));
            seg.setEpsAngle(0.2);//甘いらしい
            //seg.setEpsAngle(deg2rad(15));//15度の許容範囲
            //seg.setModelType(pcl::SACMODEL_PERPENDICULAR_PLANE);
            seg.setNormalDistanceWeight(0.1);
            seg.setMethodType(pcl::SAC_RANSAC);
            seg.setMaxIterations(50);
...
//平面の点群の表示
            ...
            *index = cluster_indices[0];
            ext.setIndices(index);
            pcl::PointCloud<PointType>::Ptr rCloud(new pcl::PointCloud<PointType>);
            ext.filter(*rCloud);

#endif
            //平面の重心を求める
            Eigen::Vector4d centroid;
            pcl::compute3DCentroid<PointType>(*rCloud, centroid);
            Eigen::Vector3d centroid1 = centroid.head(3);
            printnl(centroid1.matrix()); //回転行列部分
...
/*
RTC::ReturnCode_t PCToColorPlane::onAborting(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onError(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onReset(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onStateUpdate(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
/*
RTC::ReturnCode_t PCToColorPlane::onRateChanged(RTC::UniqueId ec_id)
{
  return RTC::RTC_OK;
}
*/
...
//引数： planes 円柱情報（入力）、seq 選択後の円柱情報（出力）
//戻り値： なし
void PCToColorPlane::selectPlanes(const std::vector<Plane>& planes, RTC::TimedDoubleSeq& seq)
{
#if 1
    //全ての候補の更新をfalseに
    list<PlaneCandidate>::iterator j;
    for (j = m_planeCandidates.begin(); j != m_planeCandidates.end(); ++j) {
        j->updated = false;
    }
    //あらたな円柱情報を順番に処理
    vector<Plane>::const_iterator i;
    for (i = planes.begin(); i != planes.end(); ++i) {
        //候補の中から1番近い円柱を求める
        float dmin = FLT_MAX;
        list<PlaneCandidate>::iterator jmin;
        for (j = m_planeCandidates.begin(); j != m_planeCandidates.end(); ++j) {
            float d = (i->center - j->plane.center).norm();
            if (d < dmin) {
                dmin = d;
                jmin = j;
            }
...
#if  0
        if (dmin != FLT_MAX) {
            print(jmin->plane.center);
            print(dmin);
            print(abs(i->radius - jmin->plane.radius));
            print(abs(i->hue - jmin->plane.hue));
        }
#endif
        //1番近い候補の円柱との距離が小さく、かつ、半径が近く、かつ、色相が近ければ、1番近い候補を更新する
        if (dmin != FLT_MAX
            && dmin < m_samePlaneCenterDistanceLimit
            && abs(i->radius - jmin->plane.radius) < m_samePlaneRadiusDistanceLimit) {
            //蓄積されたものと平均する
            jmin->plane.center = ((jmin->plane.center) * (jmin->count) + i->center) / (jmin->count + 1);
            jmin->plane.radius = ((jmin->plane.radius) * (jmin->count) + i->radius) / (jmin->count + 1);
            jmin->count++;
            jmin->updated = true;
        }
        else {
            ...
            c.count = 1;
            c.updated = true;
            m_planeCandidates.push_back(c);
        }
    }
    //更新されなかったものは削除
    for (j = m_planeCandidates.begin(); j != m_planeCandidates.end();) { //要素削除のためforの更新処理はあえて書いていない
        if (j->updated) {
            ++j;
        }
        else {
            j = m_planeCandidates.erase(j);
        }
    }
    ...
    seq.data.length(0);
    for (j = m_planeCandidates.begin(); j != m_planeCandidates.end(); ++j) {
        if (j->count >= m_planeAccumulationMin) {
            seq.data.length(5 * (k + 1));
            Eigen::Vector3f center = m_coordinateTransformation * j->plane.center;
            seq.data[5 * k + 0] = center.x();
            seq.data[5 * k + 1] = center.y();
            seq.data[5 * k + 2] = center.z();
            seq.data[5 * k + 3] = j->plane.radius;
            k++;
        }
    }
#else
    ...
    for (size_t i = 0; i < s; i++) {
        ...
        Eigen::Vector3f center = m_coordinateTransformation * planes[i].center;
        seq.data[5 * i + 0] = center.x();
        seq.data[5 * i + 1] = center.y();
        seq.data[5 * i + 2] = center.z();
        seq.data[5 * i + 3] = planes[i].radius;
        seq.data[5 * i + 4] = planes[i].hue;
    }
#endif
}
...
extern "C"
{
    ...
    void PCToColorPlaneInit(RTC::Manager* manager)
    {
        coil::Properties profile(pctocolorplane_spec);
        manager->registerFactory(profile,
            RTC::Create<PCToColorPlane>,
            RTC::Delete<PCToColorPlane>);
    }
