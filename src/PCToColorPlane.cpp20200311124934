...
RTC::ReturnCode_t PCToColorPlane::onExecute(RTC::UniqueId ec_id)
{
  ...
  if (m_pcIn.isNew()) {
    ...
    for (int i = 0; i < m_findingIterationLimit; i++) {
      ...
      //平面の推定
      seg.setOptimizeCoefficients(true);
      seg.setModelType(pcl::SACMODEL_PLANE);//変更::CYLINDER→PLANE
      seg.setMethodType(pcl::SAC_RANSAC);
      seg.setNormalDistanceWeight(0.1);
      ...
      pcl::ModelCoefficients::Ptr coefficientsPlane(new pcl::ModelCoefficients);//変更::cylinder→plane
      pcl::PointIndices::Ptr inliersPlane(new pcl::PointIndices);//変更::inliers_cylinder→inliersPlane
      seg.segment(*inliersPlane, *coefficientsPlane);
      //cout << "*coefficients_cylinder: " << *coefficients_cylinder << endl;

      //円柱の抽出
      extract.setInputCloud(cloud_filtered2);
      extract.setIndices(inliersPlane);//変更::inliers_cylinder→inliersPlane
      extract.setNegative(false);
      pcl::PointCloud<PointType>::Ptr cloudPlane(new pcl::PointCloud<PointType>());
      extract.filter(*cloudPlane);
      //cout << "cloudPlane->points.size(): " << cloudPlane->points.size() << endl;

      //点の数が少なければ処理を終了する．
      if (cloudPlane->points.size() < m_cylinderPointSizeMin) break;
      ...
      float hue = calcHue(cloudPlane, m_histogramBinNumber,
...
#if 1
      //円柱の点群の表示
      setPointCloud(m_viewer, cloudPlane, "cloudPlane" + si, true, r, g, b);
#endif
      ...
      float radius;
      reviseCylinder(cloudPlane, coefficientsPlane, center, rotation, radius);
    ...
    selectCylinders(cylinders, m_plane);
...
//戻り値： なし
void PCToColorPlane::selectCylinders(const std::vector<Cylinder> &cylinders, RTC::TimedDoubleSeq &seq)
{
#if 1
  //全ての候補の更新をfalseに
